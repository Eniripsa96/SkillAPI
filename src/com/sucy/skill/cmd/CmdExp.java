...
public class CmdExp implements IFunction
{
    ...
    public void execute(ConfigurableCommand cmd, Plugin plugin, CommandSender sender, String[] args)
    {
        ...
        if (args.length >= 1 && (args.length >= 2 || sender instanceof Player))
        {
            ...
            if (target.isOnline())
            {
                cmd.sendMessage(target.getPlayer(), RECEIVED_EXP, ChatColor.DARK_GREEN + "You have received " + ChatColor.GOLD + "{exp} experience " + ChatColor.DARK_GREEN + "from " + ChatColor.GOLD + "{player}", Filter.PLAYER.setReplacement(sender.getName()));
            }
        }
